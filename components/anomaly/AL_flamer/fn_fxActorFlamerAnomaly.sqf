...
waitUntil {sleep 5; player distance _flamer < var_flamerAnomaly_visibleDistance};

_flamer spawn
{
	while {alive _this} do
	{
		if (_this getVariable "vizibil") then
		{
			[_this, ["flamer_voice", 100]] remoteExec ["say3d"]
		};
...
{
	_part_surs = "Land_HelipadEmpty_F" createVehiclelocal [0,0,0];
	_comp_obj_casp pushBack _part_surs;
	_part_surs attachTo [_flamer, [0,0,0],_x]

} forEach _pct_flamer;

_part_array_flamer = _comp_obj_casp call fnc_flamerAnomaly_flamer_SFX;
...
while {(_flamer getVariable "vizibil") and (alive _flamer)} do
{
	...
	if (player distance _flamer < 5) then
	{
		addCamShake [5,2,5];
...
} forEach (_comp_obj_casp + _part_array_flamer);
